...
/**
*	Class to process and sort data using ForkJoin Framework, Runnable and synchronized
*/
public class DataProcessorFJ implements Runnable {
	...
	// Variable ForkJoinPool stores reference to the thread pool used for executing the merging
	...
	private void mean() {
		...
		while (true) {
			...
			try {
					...
					// Invoke mergesort using the fork-join framework
					MergeSortFJ mergeTask = new MergeSortFJ(this.MODE, vals);
					mergeFJPool.invoke(mergeTask);
					// Wait for sorted values to be obtained
					vals = mergeTask.join();
